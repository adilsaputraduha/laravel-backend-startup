...
@section('content')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            @endif
                            <a href="{{ route('blogadd') }}" class="btn btn-green mb-3">Add new</a>
                            <table id="datatable" class="table table-centered dt-responsive table-nowrap mb-0"
                                ...
                                <tbody>
                                    @foreach ($blog as $number => $data)
                                        <tr>
                                            <td>{{ ++$number }}</td>
                                            <td>{{ $data->blogTitle }}</td>
                                            <td>{{ date('d M Y', strtotime( $data->blogDate)) }}</td>
                                            <td>{{ $data->name }}</td>
                                            <td class="text-center">
                                                <a class="btn btn-green btn-sm mb-1" data-bs-toggle="modal"
                                                    data-bs-target="#editModal{{ $data->blogId }}">
                                                ...
                                                <a class="btn btn-green btn-sm mb-1" data-bs-toggle="modal"
                                                    data-bs-target="#deleteModal{{ $data->blogId }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    ...
    @foreach ($blog as $data)
        <form action="{{ route('blogdelete') }}" method="POST" enctype="multipart/form-data">
            @method('DELETE')
            @csrf
            <div class="modal" tabindex="-1" id="deleteModal{{ $data->blogId }}">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <input type="hidden" name="id" required value="{{ $data->blogId }}" />
